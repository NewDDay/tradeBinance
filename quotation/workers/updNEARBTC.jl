...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/NEARBTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("NEARBTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/NEARBTC.log", 9, "Записали NEARBTC, $(book[1].match)")
        catch
            src.logging("./quotation/now/NEARBTC.log", 0, "Не удалость записать ордербук NEARBTC")
        end
    end
end
