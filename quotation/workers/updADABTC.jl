...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/ADABTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("ADABTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/ADABTC.log", 9, "Записали ADABTC, $(book[1].match)")
        catch
            src.logging("./quotation/now/ADABTC.log", 0, "Не удалость записать ордербук ADABTC")
        end
    end
end
