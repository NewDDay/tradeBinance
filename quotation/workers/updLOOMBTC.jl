...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/LOOMBTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("LOOMBTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/LOOMBTC.log", 9, "Записали LOOMBTC, $(book[1].match)")
        catch
            src.logging("./quotation/now/LOOMBTC.log", 0, "Не удалость записать ордербук LOOMBTC")
        end
    end
end
