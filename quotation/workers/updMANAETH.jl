...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/MANAETH.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("MANAETH"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/MANAETH.log", 9, "Записали MANAETH, $(book[1].match)")
        catch
            src.logging("./quotation/now/MANAETH.log", 0, "Не удалость записать ордербук MANAETH")
        end
    end
end
