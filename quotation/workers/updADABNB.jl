...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/ADABNB.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("ADABNB"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/ADABNB.log", 9, "Записали ADABNB, $(book[1].match)")
        catch
            src.logging("./quotation/now/ADABNB.log", 0, "Не удалость записать ордербук ADABNB")
        end
    end
end
