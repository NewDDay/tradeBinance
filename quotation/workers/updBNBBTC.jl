...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/BNBBTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("BNBBTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/BNBBTC.log", 9, "Записали BNBBTC, $(book[1].match)")
        catch
            src.logging("./quotation/now/BNBBTC.log", 0, "Не удалость записать ордербук BNBBTC")
        end
    end
end
