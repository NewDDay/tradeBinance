...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/MANABTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("MANABTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/MANABTC.log", 9, "Записали MANABTC, $(book[1].match)")
        catch
            src.logging("./quotation/now/MANABTC.log", 0, "Не удалость записать ордербук MANABTC")
        end
    end
end
