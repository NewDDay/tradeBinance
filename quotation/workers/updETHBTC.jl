...
function infile(book::Vector{RegexMatch})
    open("/home/rosenrot/tradeBinance/quotation/now/ETHBTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("ETHBTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        book = wsparse(readavailable(io))
        try
            infile(book)
            #src.logging("./quotation/now/ETHBTC.log", 9, "Записали ETHBTC, $(book[1].match)")
        catch
            src.logging("./quotation/now/ETHBTC.log", 0, "Не удалость записать ордербук ETHBTC")
        end
    end
end
